...
namespace BuildingMakerToolset { 
    public class BHUtilities
    {
        ...
        public static List<Vector3> GetBoundPoints(GameObject obj )
        {
            ...
            boundList.Add( obj.transform.TransformPoint(AverageVector( forward )) );
            boundList.Add( obj.transform.TransformPoint( AverageVector( back )) );
            boundList.Add( obj.transform.TransformPoint( AverageVector( left )) );
            boundList.Add( obj.transform.TransformPoint( AverageVector( right )) );
            boundList.Add( obj.transform.TransformPoint( AverageVector( up )) );
            boundList.Add( obj.transform.TransformPoint( AverageVector( down )) );
            return boundList;
        ...
        public static Vector3 AverageVector(List<Vector3> vectors)
        {
            if (vectors.Count == 0)
                return Vector3.zero;
            Vector3 max = Vector3.zero;
        ...
        public static Vector3 BMTFindUniformScale(Vector3 baseScale)
        {
            int xScore = 0;
            int yScore = 0;
            int zScore = 0;
            if (Mathf.Abs(baseScale.x) == Mathf.Abs(baseScale.y))
            {
                xScore++;
                yScore++;
            }
            if (Mathf.Abs(baseScale.x) == Mathf.Abs(baseScale.z))
            {
                xScore++;
                zScore++;
            }
            if (Mathf.Abs(baseScale.y) == Mathf.Abs(baseScale.z))
            {
                yScore++;
                zScore++;
            }
            ...
            if (xScore >= yScore && xScore >= zScore)
                newScale = Vector3.one * Mathf.Abs(baseScale.x);
            else if (yScore >= xScore && yScore >= zScore)
                newScale = Vector3.one * Mathf.Abs(baseScale.y);
            else if (zScore >= xScore && zScore >= yScore)
                newScale = Vector3.one * Mathf.Abs(baseScale.z);
